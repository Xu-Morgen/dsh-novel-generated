...
if (lock.descriptorIds.length !== 232 || !lock.descriptorIds.includes('novel-creation-tool/novelReviewRepair/propose')) {
  throw new Error('I174 canonical registry must contain the 214-method baseline plus the existing review-repair seam');
}
if ((registrySource.match(/"key":/g) ?? []).length !== 31 || (registrySource.match(/"methodId":/g) ?? []).length !== 213) {
  throw new Error('I174 generated registry does not cover 31 Client services / 213 consumed methods');
}
...
process.stdout.write('I174 smoke: generated 31-service/213-method client, unwrap/error projection, cancellation, late-response guard, real strict IPC, and DSH-free bundle passed\n');
