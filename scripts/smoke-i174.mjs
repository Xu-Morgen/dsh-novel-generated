...
if (lock.descriptorIds.length !== 234 || !lock.descriptorIds.includes('novel-creation-tool/novelReviewRepair/propose')) {
  throw new Error('I174 canonical registry must contain the 214-method baseline plus the existing review-repair seam');
}
if ((registrySource.match(/"key":/g) ?? []).length !== 31 || (registrySource.match(/"methodId":/g) ?? []).length !== 215) {
  throw new Error('I174 generated registry does not cover 31 Client services / 215 consumed methods');
}
...
const electronEnv = { ...process.env, NOVEL_DESKTOP_SMOKE: '1', NOVEL_DESKTOP_SMOKE_HOLD_MS: '700' };
...
process.stdout.write('I174 smoke: generated 31-service/215-method client, unwrap/error projection, cancellation, late-response guard, real strict IPC, and DSH-free bundle passed\n');
