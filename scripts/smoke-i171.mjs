...
if (lock.schemaVersion !== 1 || lock.namespace !== 'desktopIpc' || lock.descriptorIds.length !== 217 || Object.keys(lock.descriptors).length !== 217 || Object.keys(lock.schemas ?? {}).length === 0) {
  throw new Error('I171 desktop IPC contract lock does not contain the 214-method baseline plus review repair');
}
if (new Set(lock.descriptorIds).size !== 217 || lock.descriptorIds.some((id) => lock.descriptors[id]?.id !== id)) {
