...
3. `docs/novel-creation-tool-development-plan.md`（v3.3）—— 执行层；**I1–I156 全部完成**。当前顺序执行 I157：来源审阅失败重试保留作者选择，且 idea/background-material/hybrid 的 POV 叙事化由系统生成稳定候选标识、由 LLM 提议并串联新主角，作者界面不暴露角色或知情信息 ID。v3.2 原 I151–I162 只作非执行 provenance，不占用当前连续迭代编号。
...
- I1–I156 已完成。当前单一执行卡为 **I157**：首次 session 创建失败后的重试必须保留作者已选来源角色、处理目标、POV 与段落裁决；`idea|background-material|hybrid + adapt-pov` 的限知路径默认由 Client 生成内部稳定 protagonist candidate ID，LLM 必须返回同 ID 的新主角候选并在 B5 中实际串联，作者只看“AI 创建/已有角色名称”，不得手填角色 ID 或知情信息 ID。既有 invocation 名称与字段保持兼容，只允许扩展 strict sourceRole 枚举并同步 contract locks；synopsis/existing-prose 边界、I151、DOCX 分段与 F1/F2 不变。每迭代单独 commit，当前验证命令为 `pnpm run verify:i157`。
