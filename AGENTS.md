...
3. `docs/novel-creation-tool-development-plan.md`（v3.3）—— 执行层；**I1–I153 全部完成**。当前顺序执行 I154：为来源角色、来源片段分类、段落处理与“合并此分类”补齐可访问的 hover/focus 解释提示。v3.2 原 I151–I162 只作非执行 provenance，不占用当前连续迭代编号。
...
- I1–I153 已完成。当前单一执行卡为 **I154**：仅在既有来源审阅 UI 为来源角色、段落来源类型、段落处理及“合并此分类”增加统一帮助按钮；鼠标 hover 与键盘 focus 均显示详细说明，原生 title 作降级。不得借 I154 改变分段、分类 enum、段落裁决、Host/Remote、prompt/schema 或 F1/F2。每迭代单独 commit，当前验证命令为 `pnpm run verify:i154`。
