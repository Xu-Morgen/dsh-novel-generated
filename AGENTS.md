...
3. `docs/novel-creation-tool-development-plan.md`（v4.1）—— 执行层；**I1–I222 / Stage 0–66 已完成，正文缺规则反馈与素材说明修复已验收交付**。v3.2 原 I151–I162 只作非执行 provenance，不占用当前连续迭代编号。
...
- 一迭代一任务：默认每次只执行一个 Ixx。2026-09-07 用户明确授权 I188–I194 UI 改造连续执行；仍逐迭代先 DoD、实现、验收、独立 commit，不混提交。
...
- I1–I222 与 Stage 0–66 已完成；I196 交付当前单 profile 明文 txt（设计 §14.35 / 需求 R37），I197 交付 LlmBackend 统一观察与受管独立窗口（设计 §14.36 / 需求 R38），I198–I199 修复初始化启动失败显示、恢复与安全 IPC 原因传递，I200 补齐视角大纲输出约束与格式失败反馈，I201 开放两次 I11 确认后的规则与文风重新生成（设计 §14.18.1a / R39），I202 交付三步生成恢复、简洁 POV 提示词及完整脱敏 txt 诊断（设计 §14.36.1 / R40），I203 交付绑定角色/节拍引用、最多两次模型修正及合并失败恢复（设计 §14.36.2 / R41），I204 修复角色更新请求与章节创建必填反馈，I205 修复正文缺规则错误透传、Client 诊断与主文案分离及素材说明（既有 strict IPC 不变），但不代表 R35 Renderer profiles 已完成。后续从 I223 另立迭代，不自动执行后置项。Renderer profile store 和新增敏感 IPC 仍必须另立后续连续迭代；不得擅自恢复后置 F1/F2。

- I206 修复章节管理刷新开始/成功未清除旧错误的问题；两项读取均成功后显示明确成功状态，真实失败保留输入，读取不写作品文件；既有领域与 IPC 不变。

- I207 在 AI 过程与错误窗口逐调用展示默认折叠的脱敏输入提示词，失败/取消保留，限长明确提示；monitor 只读投影新增可选 prompt/promptTruncated，既有领域 IPC 不变。

- I208 修复场景—细纲绑定缺项静默退出与结果反馈，显示当前正文场景、保存状态与本地回读数量；Electron 验证绑定及初始基线落盘，既有领域与 IPC 不变。

- I209 修复选择章节未回填元数据及保存静默退出；同章导航保留输入，保存结果明确可见，新建独立分配 ID/顺序；既有 Main/IPC 不变。

- I210 为范围补缺/追加/重生成携带所选解析范围已保存场景卡，按序只读参考；内部 parser input 可选新增 scopeCards，公开 IPC/输出/I11 不变。

- I211 将每次完整脱敏输入自动存档为 cache/llm-traces 下同前缀 .input.txt；失败/取消保留，凭据失败不泄漏输入，写盘故障不阻断生成；公开 IPC 不变。

- I212 增加冲突类型 hover/focus 说明，节由已保存场景卡、幕目标由节描述生成候选，经 I11 决定替换或保留；新增 strict descriptionGenerate/descriptionDecide，旧合同不变。

- I213 暂时隐藏生成队列导航入口，保留兼容路由/任务；修复章节管理变更后当前章场景列表回读，并明确普通绑定与 writing 生成基线区别；公开合同不变。

- I214 修复正文工作区场景卡候选丢弃 I19 人物/规则/文风上下文；补角色姓名表和细纲卡身份，当前导航节 writing 优先，保留卡片字数；trace/TXT 与实际注入一致，旧 IPC 不变。

- I215 将正文工作区 scene-card 限定为单卡剧情、移除节描述；两种新场景写作按目标章读取已保存正文，排除后章，超限保留最新结尾并如实标注；旧 IPC 不变。

- I216 新增 strict 场景卡草稿采用联动：保存 C5 后完成冻结 input 卡并绑定新场景，下一卡经 I11 弹窗确认/取消；B5 CAS、部分成功重试，旧采用/续写/润色接口不变。

- I217 将正文主流程改为整章阅读与一次确认定稿：Main 按已保存场景顺序分析完整正文，复用五层 writer/I11，同步已绑定且全部卡完成的节进度并标记章 canon；确认前 freshness、重开/部分失败重试、下一章/末章创建入口。新增 strict chapterManuscript/chapterAnalyze/chapterFinalize，旧合同不变；不自动重写未来细纲。

- I218 修复规则与文风长格式错误导致 running 残留、轮询失败不结束等待的问题；失败明确提示并开放同任务重试，重开恢复孤立运行状态，重生成接入流式进度。prompt 明示既有枚举，公开合同与 I11 不变；Stage 62 累积验证全绿。
