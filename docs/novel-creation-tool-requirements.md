...
3. `docs/novel-creation-tool-development-plan.md` v2.5：I1–I85 的执行卡片、交付物与命令细节，及 Stage 17 修复迭代（I86–I102）与 Stage 18 候选（I103–I112）。
...
- 当前迭代身份：**I1–I85 全部完成**；I85 DSH family `0.1.1-rc.2` 兼容升级已完成，唯一可复现项目 DSH family pin 已切换为 `0.1.1-rc.2`；I85 之后已立项 **Stage 17 修复迭代（I86–I102，review v2.0 中级以上问题，见 development-plan §18）**，Stage 18 新增功能候选（I103–I112）未排期。任何保留需求必须能追溯到其中至少一个迭代和一个精确验证命令。
...
> 定位：v2.4 期间按「专业作者视角评审」产出、并经作者逐项裁决的后续迭代立项输入（AGENTS.md §2「超范围想法记 backlog」的落地）。本表**不是当前执行范围**：I85 已完成，当前待执行为 Stage 17 修复迭代（I86–I102，review v2.0 中级以上问题，见 development-plan §18）；R18 候选已延后至全部修复迭代之后并重编号为 **I103–I112（development-plan §19）**，各行仍须经正常立项流程（填 DoD 卡、分配迭代号、加入计划）才进入执行。任何行都不改变 §0.1 宿主基线、公开 Remote/wire 契约形状与既有 13 层模型。
>
...
| R18-7 | 术语替换：工程术语（ConfirmationGate / C1–C6 / Remote / entryId / 派生索引等）全部替换为作者可理解语言，技术徽标只保留在高级视图。 | 开发计划 阶段 18（I109）；UI 文案是唯一变更面，不改变契约与数据。 | UI.3 | 未排期 |
| R18-8 | 上下文跳转链接：角色/关系/知情/审校问题/时间线节点/搜索命中/场景卡之间稳定双向跳转，返回保留上下文。 | 开发计划 阶段 18（I110）；链接信息不落盘进 Markdown 正文，作者回传后重新分析再链接，最终 txt 导出无链接可直接发布。 | UI.4 | 未排期 |
| R18-9 | 操作模式分层：正文区按「写作 / AI 候选 / 版本比较 / 场景资料」分模式展示，同一时间只展开一个辅助面板。 | 开发计划 阶段 18（I111）；解决正文区下方候选/版本/追踪/校验信息堆叠过长。 | UI.5 | 未排期 |
