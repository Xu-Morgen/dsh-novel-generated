...
> 版本：v4.1
> 日期：2026-09-04
> 状态：当前执行权威（**I1–I197 / Stage 0–41 已完成；独立 LLM 过程与错误窗口已验收交付**；v3.2 原 I151–I162 为后置 provenance，不占用当前连续编号）
...
## 30. Stage 29：来源片段裁决闭环（R31，I162）

**阶段门**：`pnpm run verify:stage-29`。本阶段只修复来源类型建议、作者分段与最终裁决闭环，不恢复后置 F1/F2。

### I162：来源处理建议与作者可控分段（R31-1）

- **依赖**：I141–I149、I154、I156、I159；**canonical owner**：来源审阅 Client controller + I143 paragraph/range schema + I142 session summary。
- **目标**：保留生成安全强依赖的五类来源语义，同时让类型建议伴随可行动的处理建议，并允许作者拆分/合并 Host 投影的来源片段后重新分析。
- **兼容/退役**：既有 Remote 方法、LLM output 与旧 session 文件保持兼容；`paragraphDecisions[].role` 仅为可选追加字段，旧调用方不发送时结果不新增该字段。退役可手选的 `edited` 与误称“合并此分类”的接受按钮；实际改分类自动记录 edited。
- **交付物**：逐字保真的 split/merge 纯函数；处理建议词典；审阅分段控件；旧 session discard + 新 session/reanalysis controller 接线；最终 role checkpoint；消费者/负向夹具；I162 smoke 产物。
- **验收**：类型与处理建议同屏；光标不能在首尾、代理对中间或来源范围外切分；合并只作用相邻片段；重分段后旧建议归零、旧 session discarded 且新分析精确一次；确认 summary 携带最终 role，旧无 role session 可读；原文、sourceHash、prompt/样本、B/C/C5、I11 与非空门不变；全量/product-flow 绿。
- **明确不做**：不编辑来源文字，不新增来源类型/处理目标/LLM 字段，不直接生成或写入任何叙事层，不开放 C5 保真导入，不实现 F1 StructuredImportSource。
- **验证**：`pnpm run verify:i162`；`pnpm run verify:product-flow`；`pnpm run verify:stage-29`；`artifacts/i162-source-segmentation-review.json`。

## 31. Stage 30：来源解释失败重试闭环（R32，I163）

**阶段门**：`pnpm run verify:stage-30`。本阶段只修复 I143/I156 来源解释任务的异步失败重试与诊断，不改变公开合同或模型语义。

### I163：异步失败任务受限原位重试（R32-1）

- **依赖**：I143、I156、I162；**canonical owner**：`ImportInterpretationAnalysisService` job lifecycle + 来源审阅 Client controller。
- **目标**：让已受理但后台 failed 的来源解释任务在相同 session/输入上可恢复，并让作者在高级详情看到第一次失败的真实原因。
- **兼容/退役**：复用既有 `begin/status/result` invocation 与全部 schema；不新增 retry Remote。`begin` 对 failed+same-input 采用受限重启语义，其他重复或错配保持拒绝；退役 Client 对 status=failed 静默丢弃错误的分支。
- **交付物**：Host job 输入等价守卫与 failed retry；Client failed-result 诊断接线；真实异步失败消费者夹具、并发/成功/错配负测；I163 smoke 与证据。
- **验收**：首次 begin 返回后后台失败，status=failed；同 session/同输入再次 begin 启动第二次且可 succeeded；queued/running/succeeded 重入以及 project/hash/paragraph 变化均拒绝；Client 不创建第二 session，普通错误保持中文，advanced detail 含原始原因；迟到响应不覆盖当前 session；全量、build、product-flow、contract lock 与 I162 smoke 全绿。
...
阶段门已通过（2026-09-07）：`pnpm run verify:stage-38`：全量测试、七张卡累积 smoke、既有 samples 与 I140/I149/I151 适用回归、Windows 打包及真实应用验收。按钮账本记录实际可达性，不以源码声明数替代。
...
- **验收 / 消费者夹具**：目录与助手跨作用域消费者、键盘/禁用负测、真实 Electron 颜色与截图；真实 Electron 生产入口截图与交互证据，不复用原型模拟结果。
- **验证**：`pnpm run verify:i188` = typecheck + pnpm test + build + 本卡 smoke；阶段末追加 `pnpm run verify:stage-38`。
...
- **明确不做**：多窗口、Renderer profiles、主题切换、关系图谱、F1/F2；不改领域/IPC、prompt/schema/样本/金标/阈值。
- **交付物 / canonical owner**：desktop shell、client presenter/nav/workflow/navigation；本卡 DoD、生产代码、回归与负向测试、smoke 证据及独立 commit。公共颜色唯一 owner 为 styles/tokens.ts，控件 styles/controls.ts，由 styles.ts 组合。
- **兼容与退役**：保留路由、data-novel 锚点、业务方法和保存/取消/重试/I11；删除被迁出的重复样式；仅本卡列出的 owner 允许最小修改。
- **验收 / 消费者夹具**：创建/打开/归档恢复、任务恢复、离开保护、窄窗无遮挡；真实 Electron 生产入口截图与交互证据，不复用原型模拟结果。
- **验证**：`pnpm run verify:i189` = typecheck + pnpm test + build + 本卡 smoke；阶段末追加 `pnpm run verify:stage-38`。
- **DoD**：实施前填 `docs/ui/ui-implementation-dod.md`，验收未通过不进入下一卡。
...
- **兼容与退役**：保留路由、data-novel 锚点、业务方法和保存/取消/重试/I11；删除被迁出的重复样式；仅本卡列出的 owner 允许最小修改。
- **验收 / 消费者夹具**：来源未决阻塞、同任务重试、部分失败如实反馈、I11 边界；真实 Electron 生产入口截图与交互证据，不复用原型模拟结果。
- **验证**：`pnpm run verify:i190` = typecheck + pnpm test + build + 本卡 smoke；阶段末追加 `pnpm run verify:stage-38`。
- **DoD**：实施前填 `docs/ui/ui-implementation-dod.md`，验收未通过不进入下一卡。
...
完成：随本卡独立提交交付；`verify:i194` / `verify:stage-38` exit 0，227 文件 / 1182 测试与真实打包作者流程通过。见 `docs/ui/ui-delivery-report.md`；后续 I195 尚未立项。
...
- **Owner**：client/source-aware-workflow、现有来源审阅/panels/presenter/workflow；desktop renderer shell 与新建同属来源流程的 typed controller/panel；Main source-import-handlers（既有领域 owner 的最小接线），已有 analyzer/plan 方法的 canonical adapter。验证缺陷限定于既有 author flow 接线，不新增领域合同。I194 消费者发现的最小修复 owner 包含 Main 新作品 C3/导入 C6 准备、host rule-style-import-initialization identity 投影、inspiration-service native LlmBackend 适配，以及现有候选导航、queue/rule-style/timeline ops 的状态保护。完整发布消费者另含 Main 显式绑定的初始 baseline 接线、writing 私有基线冻结与 finalization 成功通知（未采用/失败仍阻止发布），以及 onboarding 纯动作与历史 Remote barrel 隔离；合同/prompt/样本不变。
...
## 38D. Stage 41：统一 LlmBackend 观察与独立 AI 过程窗口（I197）

### I197：将过程观察下沉到 LlmBackend

- **目标**：在 Main LlmBackend 捕获全部生成请求的连接、推理、正文、完成、取消与失败，自动显示独立受管 Electron 窗口。
- **明确不做**：不修改领域/prompt/样本，不实现 profiles，不依赖旧业务 method 触发。
- **交付物 / owner**：Main backend observer、窗口注册表、独立 preload/HTML/React root；strict 只读事件 schema 与锁；旧同窗浮层退役；DoD、测试、真实 Electron smoke、独立提交。
- **兼容**：原 LlmBackend 请求/chunk/异常原样透传；辅助窗只消费有界脱敏投影，无领域调用能力；主窗口退出回收全部窗口，关闭观察窗不取消业务。
- **验收**：并发、取消、失败、跨片段密钥负测；来源分析实际打开第二窗口；失败状态保留；窗口隔离与生命周期。
...
## Stage 43：初始化拒绝原因 IPC 修复（I199）

- **目标 / owner**：IPC 安全错误边界及 Main 导入 adapter 仅传递已知固定初始化拒绝原因；复用 I198 Client。
- **交付物 / 验收**：见 `docs/ui/i199-dod.md`；真实 Main → registry → Client 消费者，未知异常/敏感内容负测，首次导入回归。
- **兼容 / 明确不做**：设计 §0.1.2、§14.18.2；错误 envelope 和领域合同不变，不透传任意异常、不改用户数据或 LLM。
- **验证**：`pnpm run verify:i199`；`pnpm run verify:stage-43`。
- **状态**：已完成；231 文件 / 1201 测试、构建、真实 Electron 拒绝回归及既有 smoke/样本全绿。见 `docs/ui/i199-dod.md`；下一可用迭代 I200。

## Stage 44：视角大纲输出约束与失败反馈（I200）

- **目标 / owner**：补齐 I145 prompt 嵌套合同，Main result 安全格式错误与 source plan 终态反馈。
- **交付物 / 验收**：`docs/ui/i200-dod.md`；先冻结 dev/held-out，再改 prompt，正负消费者及真实 Electron 格式失败回归。
- **兼容 / 明确不做**：设计 §14.15、§14.18，canonical schema、IPC shape、I11 不变；不自动整形、重试或写入，不更改既有样本/金标/阈值。
...
## Stage 46 / I202：叙事计划分步重试与完整调用诊断

- **目标/交付物/验收**：见 docs/ui/i202-dod.md；成功步骤复用、简洁 POV 输出、完整脱敏 txt。
...
- 验证：verify:stage-66（含 verify:i222）全绿，265 文件 / 1288 测试、累计 Electron 与旧样本通过；独立打包程序 smoke 通过，真实作品副本两种写作恢复。公开合同不变，下一可用 I223。

## Stage 65 / I221：重复角色合并（已完成）

- 目标、owner、兼容边界、恢复和验收见 `docs/ui/i221-dod.md`；按检查包工作流完成第三个独立切片。
- 明确不做：不自动合并用户作品、不修改正文历史，不用知情并集或删除关系解决冲突。
- 验证：`pnpm run verify:stage-65`（含 verify:i221）全绿，264 文件 / 1286 测试、历史样本与累计 Electron 全绿；独立目录打包和 `smoke-i221.mjs --packaged` 通过。下一可用迭代 I222。
...
- 明确不做：不自动合并角色、不改变既有 IPC、不修改真实作品；冻结/删除与跨层合并按后续切片执行。
- 验证：`pnpm run verify:stage-63`（含 verify:i219）全绿；260 文件 / 1281 测试、身份样本 4/4、实际 Electron 及历史样本全绿。下一步 I220。
...
- 明确不做：不改旧 IPC，不复制正文，不自动重写未来细纲，不自动创建或生成下一章。
- 交付物 / owner / 验收：见 `docs/ui/i217-dod.md`；Main 编排、严格新增合同、章节 UI 和真实 Electron 回归。
- 验证：`pnpm run verify:stage-61`（含 verify:i217）全绿；257 文件 / 1270 测试、真实 Electron 整章阅读/分析/取消/确认/C6 完成/下一章与末章创建入口、原有样本回归通过。下一可用 I218。
...
- **明确不做**：不删除队列后端/数据，不改绑定或细纲状态，不改领域合同。
- **交付物 / owner**：Client 导航与章节管理回读/store 目标守卫；DoD `docs/ui/i213-dod.md`。
- **验收**：真实 Electron 导航隐藏、新场景即时出现；不覆盖编辑草稿或切章结果，既有候选回归。
- **验证**：`pnpm run verify:i213` / `pnpm run verify:stage-57`；已完成，251 文件 / 1244 测试、实际 Electron 与原样本回归全绿。下一可用 I214。
...
- **明确不做**：不改 Main、IPC 或生成业务，不触碰用户作品。
- **交付物 / owner**：Client store 选择时回填、章节管理保存验证/反馈；创建兼容。DoD：`docs/ui/i209-dod.md`。
- **验收**：回填与同章草稿保留、缺项/失败反馈、保存去重、真实 Electron 改标题和视角落盘、正文不变。
- **验证**：`pnpm run verify:i209` / `pnpm run verify:stage-53`；已完成，245 文件 / 1237 测试、真实 Electron 与累计样本回归全绿。下一可用 I210。

## Stage 52 / I208：细纲绑定操作反馈

- **目标**：消除绑定细纲缺少场景/卡选择时静默返回，以及结果不在操作区域显示的问题。
- **明确不做**：不自动创建场景、选卡、改状态或生成正文，不改 Main 领域/IPC。
- **交付物 / owner**：Client bindingSave 前置检查与加载/结果状态，绑定面板显示当前选择及反馈；回读绑定与初始基线仍属既有 Main。DoD：`docs/ui/i208-dod.md`。
- **验收**：缺项零调用、去重、失败保留选择；真实 Electron 绑定持久化与基线、零正文/大纲改写；累计回归。
- **验证**：`pnpm run verify:i208` / `pnpm run verify:stage-52`。状态：已完成；244 文件 / 1235 测试、真实 Electron 与累计样本回归全绿；本地绑定/基线补充断言通过。下一可用 I209。

## Stage 51 / I207：逐调用折叠输入提示词

- **目标**：每次模型调用在 AI 过程与错误窗口有默认折叠的输入提示词。
- **明确不做**：不改生成内容或写作品，不增加输入日志，不扩大窗口权限。
- **交付物 / owner**：Main monitor 脱敏限长输入、strict 可选字段与锁、Renderer 折叠栏；旧快照兼容，领域 IPC 不变。DoD：`docs/ui/i207-dod.md`。
- **验收**：成功/失败/取消/并发、密钥解析失败、超长截断、strict 负向和真实 Electron 折叠展示；原回归全绿。
- **验证**：`pnpm run verify:i207` / `pnpm run verify:stage-51`。状态：已完成；243 文件 / 1234 测试、真实独立 Electron 与原样本回归全绿。下一可用 I208。

## Stage 50 / I206：管理刷新清理残留错误

- **目标**：修复管理刷新成功后仍显示旧操作失败的问题。
- **明确不做**：不改用户作品、领域/IPC、模型或后置功能。
- **交付物 / owner**：Client chapters-management 刷新状态机；开始清理旧消息，两项读取成功后明确反馈，真实失败继续保留输入。DoD：`docs/ui/i206-dod.md`。
- **验收**：成功清除旧错、任一读取失败不伪报成功、重试恢复/去重/输入保留/零作品写入；实际 Electron 与累计回归。
- **验证**：`pnpm run verify:i206` / `pnpm run verify:stage-50`。状态：已完成；243 文件 / 1232 测试、真实 Electron 与原样本回归全绿。下一可用 I207。

## Stage 49 / I205：正文生成缺规则反馈与素材说明

- **目标**：修复续写/按场景卡写作缺少启用规则时的笼统错误；说明非必填素材和空调和计划入口。
- **明确不做**：不绕过必需规则、不写用户作品、不改模型/样本、不新增调和计划列表方法。
- **交付物 / owner**：Main C5 适配与统一固定拒绝文案、Client 章节说明及空计划防护、unwrap/表现层将主文案与技术诊断分离；既有 ContextAssembler、strict IPC 与领域 owner 不变。DoD：`docs/ui/i205-dod.md`。
- **验收**：无启用规则零生成/零写；添加规则后二种候选成功；错误脱敏负向、实际 Electron UI 和前阶段回归。
- **验证**：`pnpm run verify:i205` / `pnpm run verify:stage-49`。状态：已完成；242 文件 / 1230 测试、真实 Electron IPC 与原样本回归全绿。下一可用 I206。

## Stage 48 / I204：角色改名与章节创建修复

- **目标**：修复角色更新携带不可变 id 被 strict IPC 拒绝，以及空章节标题/视角提交只有笼统错误的问题。
- **明确不做**：不修改公开合同、领域语义、LLM 或真实作品，不自动生成正文。
- **交付物 / owner**：Client 角色请求构造与章节管理表单；移除错误创建请求复用，校验必填字段与管理读取状态，保留输入；Main 合同与持久化 owner 不变。DoD：`docs/ui/i204-dod.md`。
- **验收**：角色改名后读盘且 ID 不变、合法章节创建、必填/读取中/重复提交负向零写、真实 Electron IPC 与全量回归。
- **验证**：`pnpm run verify:i204` / `pnpm run verify:stage-48`。状态：已完成；241 文件 / 1228 测试、真实 Electron IPC 和原样本回归全绿。下一可用 I205。

## Stage 47 / I203：角色绑定、受限修正与合并失败恢复

- 目标/交付物/验收见 docs/ui/i203-dod.md。Main 绑定 foundation 角色表，提前校验，最多两次自动纠错，合并失败定位阶段。
- 不放宽旧合同，不写作者数据，不跨重启恢复；验证 pnpm run verify:i203 / pnpm run verify:stage-47。状态：已完成；240 文件 / 1227 测试、真实 IPC/Electron、冻结样本全绿。下一可用 I204。
...
v4.1 当前进度：**Stage 37 / I187 治理、Stage 38 / I188–I194 UI 改造、Stage 39 / I195 流兼容、Stage 40 / I196 明文 Token 文件及 Stage 41 / I197 独立过程窗口均已验收交付。** I196 是当前单 profile 的作者可编辑兼容切片，I197 交付受管观察窗口，两者不等于完成 Renderer-owned 多 profile 运行时；完整 R35 实现及 packaged-app E2E 完成前仍不得标记为 v4.1 conformant。v3.2 原 I151–I162 仍为后置 F1/F2 provenance。
