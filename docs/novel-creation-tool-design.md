...
6. **细纲（v1.2）**：当前 beat 若有「细纲·场景卡」，仅注入当前场景卡摘要与目标字数，作为写作导航指令的一部分（§14.4）。
...
- **自定义单文件包**：把「非用户体验生成部分」（规则/世界观/角色/风格/大纲/细纲/关系/知情 + 正史）打包为**单个可转移分享文件**（建议 JSON/ZIP 单文件，含版本号与导出时间），可再导入重建项目（D-3）。
