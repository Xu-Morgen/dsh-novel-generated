...
I223 预算调整（§8）：B3 角色上下文上限为 8000 UTF-16 字符；基础上下文总量为 20000，完整故事上下文总量为 28000。增加的 4000 字符同步计入两级总量，其他层额度与截断/超限规则不变；输出 maxTokens 与公开合同不变。

I222 写作引用恢复（§5.5 / §5.7 / §8 / §14.15）：已完成 I11 合并的退役身份在 B5 owner 读写时解析到保留身份。只读解析不改作品字节；保存旧草稿时写入解析后引用，B5 CAS 比较同一语义视图。授权来自 accepted 合并与 lifecycle completedMergeIds 双重证据；普通冻结、未完成合并不推断身份，恢复可用的角色不自动替换。伏笔知情沿用合并选择，不扩成双方并集。合并完成后刷新已缓存 B5，保留未保存作者编辑。追加新场景只选未完成且未绑定正文的卡，使用新目标快照和绑定乐观锁，不依赖旧场景编辑基线；旧场景编辑仍保持 I121 检查。无可写卡、不可用角色、缺少知情状态通过固定脱敏文案反馈。IPC 和 prompt 不变。

I221 合并纠错增补（§5.5 / §5.10 / §14.15）：作者选择保留身份、各 B3 字段来源、C2 状态来源及 C3 知情来源，经 I11 快照预览迁移当前 B5/C1/C2/C3/C5/timeline 引用。知情、伏笔知情与关系可见性按所选身份取值，不取并集；历史正文、正史、旧状态和源角色资料保留。关系迁移产生自身关系或重复关系时阻止并说明，禁止静默删边。C3 内部身份纠正接口仅执行该确定性变换并校验前快照，普通 saveAll 单调规则不变。I11 保存前后快照，失败可重开续做；逐 owner 检查前/后状态，不覆盖中途编辑，源角色最终冻结及持久化 completedMergeIds 作为完成标记。旧公开方法不变，新增 strict characterMergePropose/Decide/Pending。

I220 角色纠错增补（§5.5 / §14.15）：B3 字段与 C2 生死语义不变，独立 character-lifecycle.yaml 保存可用/冻结/已删除状态。冻结保留历史可读并阻止修改和当前角色生成；自动生成上下文只选可用角色。角色管理经 strict additive characterManageList/Propose/Decide、引用影响预览和 I11 执行；有引用禁止直接删除，删除保留可恢复的原角色文件，旧读取可用于历史呈现。操作前重新校验角色版本、生命周期版本和引用快照；拒绝零领域写，操作标识支持同确认幂等。
...
- I166 必须先冻结 Electron、构建器、React 和目标平台的精确版本，并建立 Main/Preload/Renderer 最小启动门；在该门通过前不得迁移产品面板。
- I171–I172 必须证明 strict IPC registry、preload allowlist、参数/结果双向校验、错误信封、取消与 handler 清理；失败即停止 I173 之后的 Renderer 产品迁移。
- 开发入口可以使用构建器 dev server 作为 Renderer 资产服务，但只限开发进程；生产包不得监听网络端口或依赖外部网页。
- Electron/Chromium/Node 或打包器升级必须进入独立兼容迭代，锁文件与安全基线原子切换；不得保留旧 runtime fallback。
- I183 是生产 DSH 退役门：删除 DSH/Cordis dependencies、manifest、composition、ModuleLoader/Slot/Typert/`ctx.llm` 入口；历史测试夹具只能留在明确的 `legacy-dsh` fixture 范围，不得进入发布图。
...
| **Electron Application** | 当前外层产品/发布单元，由 Main、Preload、Renderer 组成 |
| **Legacy DSH Plugin** | I1–I164 已完成的历史宿主实现，仅作迁移来源与隔离夹具 |
| **Extension** | 创作器领域内部的可插拔能力点（Provider/Injector/Validator/Parser/规则/后端等），不等于 Electron 外层插件 |
| **Host / Client** | Main/Host 拥有数据、LLM、凭据与业务能力；Renderer/Client 只拥有 UI |
...
| **宿主与所有权唯一** | Electron 是唯一宿主；Main 持有业务与 I/O，Renderer 只持有 UI，所有副作用随 DesktopLifecycle 回收 |
...
本项目只有一个外层产品身份：Electron Application。以下接口是该产品**内部的小说领域 Extension points**，不是新的 Electron 插件类型，也不赋予 Extension 独立进程、窗口、文件、凭据、LLM 或 UI 所有权。
...
所有 Extension 通过 Main/ApplicationKernel 定义的注册表与 Service 合同接入，受同一校验、事务和 DesktopLifecycle 管理；需要 UI 的 Extension 只能贡献产品允许的受控 Renderer view model，不能自建窗口、preload bridge 或 IPC channel。
...
| # | 决策点 | 候选 | 定稿 |
|---|---|---|---|
| D1 | 产品宿主与交付形态（历史） | 独立自建应用 / DSH 原生插件 | ⏹ **v2.0 历史裁决，已由 D34 取代**：I1–I164 依此完成 DSH 插件交付；自 v4.0 起不再是当前宿主权威。 |
...
| **M21** | Electron 权威与最小运行时（Stage 32–33，I165–I172） | 桌面基线、版本 pin、ApplicationKernel、路径、CredentialStore、LlmBackend、strict IPC 与安全 preload 成立 |
| **M22** | Renderer 与作者工作流迁移（Stage 34，I173–I180） | 独立 React root、Store、全部 31 个服务消费面与 README 十二步流程迁入桌面壳 |
...
I213 用户界面调整：暂时隐藏生成队列导航入口，保留队列服务、任务与兼容路由供后续恢复；章节管理 mutation 完成后回读当前章节的场景投影，按章节身份防止迟到覆盖切章结果，不重置正文编辑缓冲。细纲卡是规划，正文场景是章节内实际保存的正文单元；普通新场景候选接受自动创建 C5 场景，但不自动改 B5 状态或建立手动绑定，定稿基线保持既有显式流程。

I214 按场景卡写作修复：正文工作区将同次 Main 装配的 I19 sources 传入候选生成，包含 B3 人物设定、规则、文风、POV 已过滤知识及近期上下文；复用既有预算和裁剪，另附不截断的紧凑 ID/姓名/别名/角色类型表以保留姓名身份。提示词明确当前细纲卡 ID、状态、标题、摘要、全部要点、POV 与卡片保存字数，不以全局字数替换场景卡目标。当前导航节内优先 writing 卡，否则沿用原顺序；不改变 C6 导航或编辑器选中状态。trace 复用实际注入层摘要，I211 TXT 自动记录实际输入。既有 IPC 不变；旧内部 I43 无 sources 调用保持兼容，不构成正文工作区的降级路径。

I216 场景卡采用联动（用户授权）：新 strict sceneCardDraftAdopt 在旧 C5-only adoptDraft 成功后，以本次生成冻结的唯一细纲卡身份完成 B5 状态并建立 C5 场景绑定，绝不使用 Renderer 当前选卡；B5 使用内部 compare-and-save 防止覆盖作者修改，部分成功明确返回可重试状态，不重复正文。随后按 canonical B5 顺序提出下一张未完成卡的 I11 确认，弹窗展示标题/摘要/要点与字数，确认后才设 writing，取消保持原状态。旧 adoptDraft、续写和润色语义不变；不自动生成下一段或定稿，不批改其他 writing 卡。当前导航节的所有卡完成后，后续 scene-card 可根据作者已设 writing 的卡重定位生成导航，不写 C6。新增两个 additive descriptor，既有调用不变；记录随项目 I11 保存，候选采用重试上下文仍属本次 Main 会话。

I215 单卡与承接范围：正文工作区 scene-card 仅将目标卡作为待写剧情，移除 I43 指令中的节导航文案及 I19 Outline 段；导航仍用于内部选卡/一致性校验，不把后续事件作为模型参考。要求在本卡范围内展开动作、对话和感官细节，不重述已经完成的事件。两种新场景候选（continue / scene-card）由既有输入 chapterId 限定历史：读取目标章全部非空当前 C5 正文及此前最多三个非空场景，按章/场景叙事顺序排除后章；未接受候选、未选分支、空场景均不充当正文。History 沿用 5000 UTF-16 字符预算：可容纳时提供所选正文全文，超限保留末尾最多 1200 字符的保存摘要及最新正文尾部，明确标记截断，trace 如实报告；不以模型另生成摘要，不改变保存内容。旧无目标章调用维持原历史选择，IPC/持久化合同不变。
...
- 段落来源类型是生成安全强依赖：`world-truth`、`author-instruction` 与 `presentation-note` 分别控制幕后事实、创作命令和呈现约束不得逐字进入读者可见 B5/C4/C5；因此不得删除该轴。
- 系统在建议来源类型时必须同步显示该类型的确定性后续处理建议。处理建议只解释既有投影边界，不新增 LLM 输出字段，不替作者接受或排除片段。
...
```text
src/desktop/main/       Electron 启动、BrowserWindow、安全策略、菜单、协议与应用生命周期
src/desktop/preload/    contextBridge 与版本化 novelDesktop API
src/desktop/renderer/   React root、桌面 shell、Store adapter、IPC client
src/app/                framework-neutral ApplicationKernel、DesktopLifecycle ports
src/platform/           credentials、LLM、paths、IPC、OS integration adapters
src/core/               领域核心与文件仓储；不得 import electron/renderer
src/host/               迁移期领域 Service 来源；逐步去除 Cordis 组合语义
```

依赖只允许 `desktop → app/platform → host/core/llm`；`core/llm` 不得反向 import `electron`、`desktop` 或具体凭据/IPC实现。Preload 与 Renderer 只能 import 可序列化合同和 Client 纯模块。
...
### 14.34 Electron 暖纸 UI 改造（Stage 38 / I188–I194，R36）

2026-09-07 授权：以当前 Electron 生产源码和领域合同为准落实 UI-A～UI-F。I187 治理已提交（f489a83）；本节不改变 §0.1，不实现多窗口、Renderer profiles、主题切换、关系图或 F1/F2；当前包仍不宣称 v4.1 conformant。

- 暖纸 #F4F1EA、墨色 #262923、朱砂 #A83D30，成功/信息/警告/危险独立语义色；系统字体，无网络资产。唯一 token owner 为 `src/client/styles/tokens.ts`，公共控件为 `styles/controls.ts`，desktop renderer.css 只管理宿主页几何。旧 --nv-* 名称兼容但不转发 DSH 主题。
- 作品、当前任务与正文为视觉主体；每个当前任务区至多一个醒目主操作。合并章节/场景导航，保留四模式及全部稳定路由；助手按需展开，迁移从作品库进入。宽窗正文面宽 640–760px，18px/1.9；窄窗堆叠，不遮挡操作与焦点。
- 覆盖导入审阅、初始化、候选/版本/定稿、资料/工具、设置及全局异常。按钮清单逐项校正并建实施账；不可复制原型固定数据或模拟回调。
- 保存失败保留输入、离开保护、来源未决阻塞、候选裁决、I11 ConfirmationGate、删除影响、归档只读及队列控制继续由现有 owner 决定。外观不能放宽条件或隐藏失败。
- 设计输入 `docs/ui/2026-09-07-ui-*` 为参考；89 项原型检查不是应用验收。逐卡确定性/负向回归、真实 Electron 截图及 smoke；阶段末全量与适用既有 held-out、累积门、真实 Windows 打包验收。

Stage 38 已验收交付（2026-09-07），证据见 `docs/ui/ui-delivery-report.md`。执行细分：I193 收口设置/辅助/全局状态；I194 完成 §14.15 既有来源到叙事导入计划的生产 UI 接线与最终作者流程/打包验收。不得用旧六层分析替代来源感知投影，不涉及后置 F1/F2，也不新增领域或 IPC 合同。

I194 接线边界补充（§14.34）：普通来源复用已锁定 I52/I56 六层审阅；视角重构复用 I148 NarrativeImportPlan，只使用已确认来源，B5/C3 由对应 owner 产生，未确认开场公开的来源不成为 C4。新作品由 Main 通过 C3 owner 建空文档；确认导入后通过现有 C3/C6 owner 补齐新角色空知情状态和缺失起始游标，保留既有状态并拒绝损坏数据。新场景候选只要求 strict 正文 preview，接受后选择实际写入场景，结构预览留在独立定稿；重写原有预览不变。灵感服务使用 Main native LlmBackend 和受控设置，原 prompt/结果 schema/I11 apply 保持。计划编辑、规则文风、时间线的未保存输入复用现有离开保护；队列重试明确为重新排队后启动同一任务。 显式保存写作中细纲卡的场景绑定时，由既有 baseline owner 建立缺失的首个意图快照；已有 stale 快照不自动替换。rewrite 仅冻结已有 fresh 基线 ID，不改变 prompt 和生成零写；完整定稿应用后由 coordinator 通知 writing 会话 owner 结清同场景已采用候选，未采用候选、失败和未确认继续阻止发布。onboarding 纯动作与历史 Remote barrel 隔离，生产只导入 actions/type，不恢复退役依赖。
...
在上述有界窗口投影之外，Main 将每次脱敏后的 LlmBackend 增量与完整正文写入 userData/cache/llm-traces 的 stream.txt / result.txt。传输结束附加可识别输出的 JSON/schema 诊断（路径、错误码、预期类型，无模型值或原始异常）；该诊断不替代业务语义校验。stream/result 不含输入提示词或 provider 配置，密钥统一脱敏，不进入作品/导出包；无静默截尾，记录失败提示且不阻断模型；全部记录生命周期归 LlmMonitor/DesktopLifecycle。用户可在关闭应用后清理，模型正文会明文保留用于本机调试。

I211 补充：Main 在凭据解析成功并对完整输入统一脱敏后、provider 调用前，自动写入 `cache/llm-traces/<同次调用前缀>.input.txt`（UTF-8），与 `.stream.txt` / `.result.txt` 配对。输入不受 monitor 预览限长影响；成功、失败与取消均保留。凭据解析失败不保存原始输入；记录写盘失败沿用固定提示并与生成隔离。不追溯历史内存调用，不改变 IPC、模型输入或作品持久化。
