...
async function makeService(projectsRoot: string, seen: string[], full = false) {
  ...
  return createWorkspaceEditorService({ characters: dummy, worldview: dummy, outline: dummy, relationship: dummy, state, canon, confirmation, projects: dummy, upload: dummy, text, textEdit });
}
