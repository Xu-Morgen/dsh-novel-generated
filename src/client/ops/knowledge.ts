...
export function createKnowledgeOps(runtime: OpsRuntime, port: KnowledgePort): KnowledgeEditOps {
  const { act, snapshot, beginOp, endOp, isActive } = runtime;
  const projectId = runtime.projectId;
  const workspace = port.workspace;
  const knowledgeNamespace = port.knowledgeNamespace;
      ...
      return {
        ...
        propose(kind: 'reveal' | 'holder-add'): void {
          ...
          if (state.selectedEntryId === undefined || state.draft.holders.length === 0 || state.busy.propose === true) return;
          ...
          knowledgePatch({ busy: { ...snapshot.knowledge.busy, propose: true }, message: undefined });
        ...
        accept(proposalId: string): void {
          const target = knowledgeNamespace;
          if (!target || projectId === undefined || snapshot.knowledge.busy.accept === true) return;
          if (!beginOp(`knowledge:accept:${proposalId}`)) return;
          const release = (): void => endOp(`knowledge:accept:${proposalId}`);
          knowledgePatch({ busy: { ...snapshot.knowledge.busy, accept: true }, message: undefined });
