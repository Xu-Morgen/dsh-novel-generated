...
/** 渲染创作设置表单：目标字数 + 内容不足时询问开关 + 保存 + 状态行。 */
export function workbenchSettingsPanel(
  ...
  return h('section', { className: 'nv-panel nv-settings', 'data-novel-workbench-settings': '', 'data-novel-layer-state': 'ready' },
    ...
    h('button', { type: 'button', className: 'nv-btn', 'data-novel-workbench-save': '', disabled: namespace === undefined || draft.saving, onClick: () => save() }, draft.saving ? '保存中…' : '保存设置'),
