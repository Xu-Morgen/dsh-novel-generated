...
export function relationshipLayer(h: El, _projectId: string, _workspace: WorkspaceNamespace | undefined, characters: readonly CharacterShape[], layerState: RelationshipLayerState, editor: RelationshipEditor, ops: RelationshipEditOps): unknown {
  // 关系 from/to 持久化为角色 id（B3 稳定主键，改名不换 id）；显示 join B3 角色名，
  // 未知 id 回退显示 id 本身（角色尚未建立 / 已被删除时仍可辨认）。
