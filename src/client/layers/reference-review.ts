...
const OWNER_LABEL: Record<ReferenceAuditOwner, string> = { c1: '关系', c3: '知情', c4: '正史' };
...
function targetText(target: ReferenceAuditRecord['targets'][number]): string {
  return `${OWNER_LABEL[target.owner]} · ${target.field} · ${target.entityId}`;
}
...
export function referenceReviewPanel(
  ...
  if (namespace === undefined) {
    return h('section', { className: 'nv-panel', 'data-novel-reference-review-panel': '', 'data-novel-reference-review-state': 'error', role: 'alert' }, '引用审查功能暂时不可用，请稍后重试。');
  }
  ...
  return h('section', { className: 'nv-panel nv-reference-review', 'data-novel-reference-review-panel': '', 'data-novel-reference-review-state': state.status },
    h('div', { className: 'nv-panel__header' },
      h('div', null,
        ...
        h('p', { className: 'nv-panel__hint' }, '查看自动更新的关系、知情与正史证据；标记仅供本次审查，修正必须先生成候选并确认。'),
      ),
    ...
    state.status === 'error' ? h('p', { className: 'nv-editor__error', 'data-novel-reference-audit-error': '', role: 'alert' }, toUserMessage(state.message ?? '引用审查记录读取失败')) : null,
    state.status === 'ready' && records.length === 0 ? h('p', { className: 'nv-outline__nodetail', 'data-novel-reference-audit-no-match': '' }, '当前筛选没有记录。') : null,
    h('div', { className: 'nv-reference-review__records', 'data-novel-reference-audit-records': '', role: 'list' }, records.map((record, index) => {
      ...
      return h('article', { key: record.recordId, className: `nv-reference-review__record${marked ? ' is-marked' : ''}`, 'data-novel-reference-audit-record': record.recordId, role: 'listitem' },
        h('div', { className: 'nv-reference-review__record-header' },
          h('strong', null, `${sourceLabel(record.source)} · ${STATUS_LABEL[record.status]} · 记录 ${index + 1}`),
          advancedReference(h, '查看记录标识', record.recordId),
        ),
        h('p', { className: 'nv-reference-review__record-meta' }, `第 ${record.attempt} 次尝试`, advancedReference(h, '查看操作标识', record.operationId)),
        record.error === undefined ? null : h('p', { className: 'nv-editor__error', role: 'alert' }, toUserMessage(record.error)),
        ...
        h('button', { type: 'button', className: 'nv-btn nv-btn--choice', 'data-novel-reference-audit-mark-error': record.recordId, 'aria-pressed': marked, onClick: () => ops.toggleError(record.recordId) }, marked ? '取消错误标记' : '标记为需检查'),
      );
    })),
    ...
    h('section', { className: 'nv-reference-review__correction', 'data-novel-reference-correction': '', 'data-novel-reference-correction-state': state.correctionStatus },
      ...
      h('button', { type: 'button', className: state.correctionCandidate === undefined ? 'nv-btn nv-btn--primary' : 'nv-btn', 'data-novel-reference-correction-propose': '', onClick: ops.proposeCorrection, disabled: correctionNamespace === undefined || state.markedErrors.length === 0 || state.correctionInstruction.trim() === '' || state.correctionStatus === 'proposing' || state.correctionStatus === 'acting' }, state.correctionStatus === 'proposing' ? '正在生成候选…' : '生成修正候选'),
      ...
      state.correctionCandidate === undefined ? null : h('article', { className: 'nv-reference-review__candidate', 'data-novel-reference-correction-candidate': state.correctionCandidate.candidateId },
        h('p', { className: 'nv-reference-review__record-meta' }, `引用修正候选 · 可信度：${state.correctionCandidate.confidence}`),
        h('p', { className: 'nv-panel__hint' }, state.correctionCandidate.rationale),
        h('ul', { className: 'nv-reference-review__targets', 'data-novel-reference-correction-preview': '' }, state.correctionCandidate.preview.map((item) => h('li', { key: `${item.owner}:${item.entityId}:${item.field}` }, `${OWNER_LABEL[item.owner]} · ${item.entityId} · ${item.field}：${JSON.stringify(item.before)} → ${JSON.stringify(item.after)}`))),
        h('div', { className: 'nv-reference-review__candidate-actions' },
          h('button', { type: 'button', className: 'nv-btn nv-btn--primary', 'data-novel-reference-correction-accept': state.correctionCandidate.candidateId, onClick: ops.acceptCorrection, disabled: correctionNamespace === undefined || state.correctionStatus === 'acting' }, '确认并应用'),
          h('button', { type: 'button', className: 'nv-btn nv-btn--ghost', 'data-novel-reference-correction-reject': state.correctionCandidate.candidateId, onClick: ops.rejectCorrection, disabled: correctionNamespace === undefined || state.correctionStatus === 'acting' }, '不采用候选'),
        ),
      ),
    ...
    h('small', { className: 'nv-panel__hint', 'data-novel-reference-audit-project': projectId }, '记录来自系统操作记录；未生成候选时此面板不执行引用修正写回。'),
  );
}
