...
function managementPanel(h: El, state: ChaptersLayerState, ops: ChaptersEditOps, choices: ChaptersEntityChoices): unknown {
  ...
  return h('div', { className: 'nv-chapters__management', 'data-novel-chapter-management': '', 'data-novel-management-state': management.status },
    ...
    h('div', { className: 'nv-editor__actions' },
      h('button', { type: 'button', className: 'nv-btn', 'data-novel-chapter-create': '', disabled: management.status === 'loading', onClick: () => ops.createChapter() }, management.status === 'loading' ? '处理中…' : '新建章节'),
      h('button', { type: 'button', className: 'nv-btn', 'data-novel-management-refresh': '', disabled: management.status === 'loading', onClick: () => ops.refreshManagement() }, '刷新管理状态'),
    ),
    ...
    h('p', { className: 'nv-chapters__item-meta' }, '先填写章节标题并选择视角角色，再点击“新建章节”。创建后在左侧选中章节，切到“候选”生成正文。'),
    h('div', { className: 'nv-editor__actions' },
      h('button', { type: 'button', className: 'nv-btn', 'data-novel-chapter-update': '', disabled: management.status === 'loading', onClick: () => ops.updateChapter() }, '保存章节信息'),
    ...
    chapter !== undefined ? h('div', { className: 'nv-chapters__management-scene-form', 'data-novel-scene-management': '' },
      ...
      h('p', { className: 'nv-chapters__item-meta', 'data-novel-scene-management-help': '' }, '这里用于手工新建或修改章节内的场景。首次生成正文只需选中章节，不必先新建空场景；场景摘要是便于识别场景的简短说明，可以留空。'),
    ...
    h('div', { className: 'nv-chapters__binding', 'data-novel-scene-outline-binding': '' },
      ...
      h('p', { className: 'nv-chapters__item-meta' }, '细纲卡是写作计划，场景是章节内实际保存的正文片段。按场景卡写作并接受为草稿时会自动创建场景，无需逐卡手工新建空场景。此处绑定用于关联已有正文与细纲，不会生成正文。'),
      h('p', { className: 'nv-chapters__item-meta' }, '普通绑定不要求细纲卡先设为“写作中”；该状态用于在保存绑定时建立后续定稿同步需要的生成基线。'),
      h('p', { className: 'nv-chapters__item-meta', 'data-novel-binding-selection': '' }, state.selectedSceneId === undefined ? '当前未选中正文场景。请在左侧展开章节，点击其中的具体场景。' : `当前正文场景：${selectedScene?.summary || '已选中场景（无摘要）'}`),
      ...
      management.binding?.message ? h('p', { className: management.status === 'error' ? 'nv-error' : 'nv-chapters__item-meta', role: management.status === 'error' ? 'alert' : 'status', 'data-novel-binding-message': '' }, management.binding.message) : null,
      h('div', { className: 'nv-editor__actions' },
        h('button', { type: 'button', className: 'nv-btn', 'data-novel-binding-save': '', disabled: management.binding?.status === 'loading', onClick: () => ops.bindingSave() }, management.binding?.status === 'loading' ? '处理中…' : '绑定细纲'),
        h('button', { type: 'button', className: 'nv-btn', 'data-novel-binding-rebind': '', disabled: management.binding?.status === 'loading', onClick: () => ops.bindingRebind() }, '更换细纲绑定'),
        h('button', { type: 'button', className: 'nv-btn', 'data-novel-binding-unbind': '', disabled: management.binding?.status === 'loading', onClick: () => ops.bindingUnbind() }, '解除细纲绑定'),
      ),
...
function reconciliationPanel(h: El, state: ChapterManagementState['reconciliation'], ops: ChaptersEditOps, planOptions: readonly EntityOption[]): unknown {
  ...
  return h('div', { className: 'nv-chapters__reconciliation', 'data-novel-outline-reconciliation': '', 'data-novel-reconciliation-state': state.status },
    ...
    planOptions.length === 0 && plan === undefined ? h('p', { className: 'nv-chapters__item-meta', 'data-novel-reconciliation-no-options': '' }, '此入口当前没有可选择的调和计划，也不影响首次生成正文。接受正文草稿并修改保存后，可在“正文”的定稿同步中点击“分析最终正文”，审阅需要调整的后续细纲。') : null,
    h('div', { className: 'nv-editor__actions' },
      h('button', { type: 'button', className: 'nv-btn', 'data-novel-reconciliation-read': '', disabled: state.planId.trim() === '', onClick: () => ops.reconciliationRead() }, '查看调整影响'),
