...
export function timelinePanel(
  ...
  const detail = h('div', { className: 'nv-editor__detail' },
    ...
    h('div', { className: 'nv-form' },
      h('button', { type: 'button', className: 'nv-btn', 'data-novel-timeline-set-current': node.id, onClick: () => ops.setCurrent(current === node.id ? null : node.id) }, current === node.id ? '恢复自动时间点' : '设为当前时间点'),
    ...
    h('div', { className: 'nv-editor__actions' }, h('button', { type: 'button', className: 'nv-btn nv-btn--primary', 'data-novel-timeline-save': '', onClick: ops.save, disabled: !state.dirty || state.saving }, saveButtonLabel(state.saving, '保存时间点'))),
