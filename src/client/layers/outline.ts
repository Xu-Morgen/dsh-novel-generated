...
export interface OutlineActShape { id: string; index: number; title: string; goal: string; beats: OutlineBeatShape[]; [key: string]: unknown; }
export interface OutlineBeatShape { id: string; title: string; description: string; charactersInvolved: string[]; conflictType: string; prerequisites: string[]; optional: boolean; detailBeats: OutlineDetailBeatShape[]; [key: string]: unknown; }
export interface OutlineDetailBeatShape { id: string; title: string; summary: string; pov: string; wordTarget: number; points: string[]; status: string; [key: string]: unknown; }
export interface OutlineShape { id: string; structure: string; logline: string; themes: string[]; acts: OutlineActShape[]; foreshadowing: unknown[]; endings: unknown[]; version?: number; [key: string]: unknown; }
...
export interface OutlineEditor { draft: OutlineShape; dirty: boolean; error: string; selectedActId: string | undefined; selectedBeatId: string | undefined; selectedDetailId: string | undefined; }
  ...
  return h('div', { className: 'nv-outline__card-editor', 'data-novel-detail-card-editor': card.id },
    ...
    h('div', { className: 'nv-form' },
      ...
      h('label', { className: 'nv-field' },
        ...
        h('select', { className: 'nv-field__input', value: card.status, onChange: (event: { target: { value: string } }) => setDetail((item) => ({ ...item, status: event.target.value })) },
          ['planned', 'writing', 'done'].map((value) => h('option', { key: value, value }, value))),
      ),
