...
function detailBeatEditor(h: El, card: OutlineDetailBeatShape, characterOptions: readonly EntityOption[], setDetail: (update: (item: OutlineDetailBeatShape) => OutlineDetailBeatShape) => void, remove: () => void): unknown {
  return h('div', { className: 'nv-outline__card-editor', 'data-novel-detail-card-editor': card.id },
    h('h4', { className: 'nv-outline__subtitle' }, `细纲场景卡：${card.title || '未命名场景卡'}`),
    h('div', { className: 'nv-form' },
      ...
      entitySelect(h, '视角角色', card.pov, characterOptions, (value) => setDetail((item) => ({ ...item, pov: value })), 'outline-detail-pov'),
      ...
      h('div', { className: 'nv-editor__actions' }, h('button', { type: 'button', className: 'nv-btn nv-btn--danger nv-btn--compact', 'data-novel-detail-remove': card.id, onClick: remove }, '删除当前细纲卡')),
    ),
  );
}
...
export function outlineLayer(h: El, _projectId: string, _workspace: WorkspaceNamespace | undefined, layerState: OutlineLayerState, editor: OutlineEditor, ops: OutlineEditOps, characterOptions: readonly EntityOption[] = [], links?: ContextLinkSink, generation?: OutlineDetailGenerationView): unknown {
  ...
  const beatPanel = beat === undefined ? h('div', { className: 'nv-outline__detail' }, h('h3', { className: 'nv-editor__title' }, '节'), h('p', { className: 'nv-outline__nodetail' }, '选择或新建一节以编辑节与细纲场景卡。')) : h('div', { className: 'nv-outline__detail' }, h('h3', { className: 'nv-editor__title' }, `节：${beat.title || '未命名节'}`), h('div', { className: 'nv-form' },
    ...
    h('div', { className: 'nv-editor__actions' }, h('button', { type: 'button', className: 'nv-btn', 'data-novel-outline-add-detail': '', onClick: () => ops.addDetailBeat(act!.id, beat.id) }, '新建细纲场景卡')),
  ...
  return h('section', { className: 'nv-editor', 'data-novel-layer-panel': 'outline', 'data-novel-layer-state': 'ready' },
    h('div', { className: 'nv-outline__toolbar' },
      ...
      h('div', { className: 'nv-editor__actions' }, h('button', { type: 'button', className: editor.dirty ? 'nv-btn nv-btn--primary' : 'nv-btn', 'data-novel-outline-save': '', disabled: !editor.dirty || editor.saving, onClick: ops.save }, saveButtonLabel(editor.saving, '保存大纲'))),
    ),
    h('div', { className: 'nv-outline__columns' },
      h('div', { className: 'nv-editor__list' },
        h('div', { className: 'nv-editor__toolbar' }, h('button', { type: 'button', className: 'nv-btn', 'data-novel-outline-add-act': '', onClick: ops.addAct }, '新建幕')),
        editor.draft.acts.map((entry) => h('div', { key: entry.id, className: 'nv-outline__act' },
          h('button', { type: 'button', className: 'nv-editor__item' + (editor.selectedActId === entry.id ? ' is-active' : ''), 'data-novel-outline-act': entry.id, onClick: () => ops.selectAct(entry.id) }, `幕${entry.index} · ${entry.title || '未命名幕'}`),
          h('button', { type: 'button', className: 'nv-btn nv-btn--danger nv-btn--compact', 'data-novel-outline-remove-act': entry.id, onClick: () => ops.removeAct(entry.id) }, '删除幕'),
          h('div', { className: 'nv-outline__beats' }, entry.beats.map((entryBeat) => h('button', { key: entryBeat.id, type: 'button', className: 'nv-editor__item nv-outline__beat' + (editor.selectedBeatId === entryBeat.id ? ' is-active' : ''), 'data-novel-outline-beat': entryBeat.id, onClick: () => ops.selectBeat(entry.id, entryBeat.id) }, `节 · ${entryBeat.title || '未命名节'}`)), h('button', { type: 'button', className: 'nv-btn nv-btn--ghost', 'data-novel-outline-add-beat': entry.id, onClick: () => ops.addBeat(entry.id) }, '新建节'), h('button', { type: 'button', className: 'nv-btn nv-btn--danger nv-btn--compact', 'data-novel-outline-remove-beat': entry.id, onClick: () => ops.removeBeat(entry.id, editor.selectedBeatId ?? '') }, '删除节')),
        )),
      ),
