...
- 权威设计文档：`docs/novel-creation-tool-design.md`（v3.0）
- 权威开发计划：`docs/novel-creation-tool-development-plan.md`（v3.0；I1–I140 已完成，当前执行 I141，Stage 19 排期至 I149）
- 需求与验收矩阵：`docs/novel-creation-tool-requirements.md`（v3.0）
...
> **交付状态**：I1–I140 已完成，以上 12 步主流程、细纲范围生成、统一定稿、全书完成门、单文件合稿和作者流程界面均已交付。当前执行 I141；I141–I149 正在增强步骤 1–2 的来源解释、幕后素材 POV 叙事化、C3 秘密揭示计划与已有正文保真导入，在完成前不得把这些新增分支表述为已经可用。

### 完整产品级测试流程

| 步骤 | 必须观察到的作者结果 | 主要迭代证据 |
| --- | --- | --- |
| 1–2 解释来源并确认大纲 | 已交付思路/梗概/长稿拆纲；新增路径还须区分幕后素材、故事计划、已有正文与混合文档，绑定作者确认的目标处理方式/POV/揭示意图；候选确认前零写，非空作品不被静默覆盖 | I37–I38、I50–I53、I119–I120；增强 I141–I149 |
...
| 主流程整体 | 默认进入作者流程，技术工具不干扰主线；固定 fake LLM 端到端覆盖成功、拒绝、陈旧和失败路径；来源感知增强完成后仍汇入同一流程 | I139–I140；增强 I149，统一运行 `pnpm run verify:product-flow` |
...
- I1–I140 已交付完整 12 步主流程；当前 I141–I149 只新增导入阶段的来源解释、幕后素材叙事化和已有正文保真导入。现有导入在 Stage 19 完成前仍可能把混合幕后资料按普通长稿拆成大纲。
