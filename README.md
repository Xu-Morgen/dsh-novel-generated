...
故事资料、读者体验大纲、秘密揭示计划分别调用模型。同一页面与来源下，点击“重试未完成步骤”会复用已成功的 Main 任务，只重新请求失败或取消的步骤，然后继续后续流程。切换来源或重启应用后需要重新生成。读者体验初稿只规划核心节拍，细纲在后续按范围单独生成。

大纲使用故事资料已生成的角色 ID/姓名清单，揭示计划使用实际节拍 ID。大纲与揭示输出不通过校验时，会额外调用模型修正，最多两次，并显示“正在修正…第 1/2 次”。引用修正只允许修改程序指出的位置和合法 ID，其他剧情内容保持原样；无法解析或其他校验失败则重生成当前阶段。仍失败时保留上游结果，等待手动重试。整套计划合并失败会说明失败层，重试会清理该层及下游结果，避免反复复用错误候选。所有结果通过后仍需确认才写入作品。

每次模型调用在 Electron `userData/cache/llm-traces/` 保存一组 UTF-8 文件（文件名含时间与请求编号）：

- `*.stream.txt`：按顺序记录解码后的正文/推理增量、时间、结束状态及可识别的故事资料/大纲/揭示输出的结构诊断。
- `*.result.txt`：脱敏后的完整最终正文；调用中断时保留已收到的部分。

过程窗口仍只显示最后 16000 字符，排查时请使用这两个完整文件。`schema-valid` 仅表示结构通过，来源回引、知情边界等语义校验及最终写入仍由业务服务判定。记录不含请求 prompt、endpoint、访问密钥或原始异常，但包含模型返回的故事内容；记录保留在本机，可在应用关闭后手动清理。
