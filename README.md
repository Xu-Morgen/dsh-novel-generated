...
- 权威设计文档：`docs/novel-creation-tool-design.md`（v3.3）
- 权威开发计划：`docs/novel-creation-tool-development-plan.md`（v3.3；I1–I150 已完成，当前执行 I151 首次导入规则/文风初始化；v3.2 原 I151–I162 已后置）
- 需求与验收矩阵：`docs/novel-creation-tool-requirements.md`（v3.3）
...
> **交付状态**：I1–I150 已完成，以上 12 步主流程、范围细纲生成修复、统一定稿、全书完成门、单文件合稿、来源确认、幕后 POV 叙事化与 C3/C4 安全边界均已交付。当前执行 I151：在作品首次导入时一次性生成规则/文风初稿并落地本地文件。v3.2 原 I151–I162 导入地基/正文保真卡片已后置；当前不把已有正文表述为已可保真进入 C5。

### 完整产品级测试流程

| 步骤 | 必须观察到的作者结果 | 主要迭代证据 |
| --- | --- | --- |
| 1–2 解释来源并确认大纲 | 步骤 1 导入来源并确认来源角色、当前目标处理以及适用 POV/揭示意图；步骤 2 按确认意图生成大纲候选并审阅，幕后素材形成读者体验与揭示计划；I151 将仅在首次导入事件同时生成可编辑的规则/文风初稿，经确认写入 `rules/*.yaml`/`style.yaml`，重开应用不再调 LLM；已有正文仍只进入既有拆纲，不直接进入 C5 | I37–I38、I50–I53、I119–I120；增强 I141–I149、I151 |
| 3 按范围生成细纲 | 当前选中节无需手填 ID；可输入生成要求并追加新的 LLM 候选，逐卡选择是否保留；只改选中范围，已有卡与稳定 ID 默认受保护 | I133–I134、I150 |
...
| 主流程整体 | 默认进入作者流程，技术工具不干扰主线；固定 fake LLM 端到端覆盖成功、拒绝、陈旧和失败路径；来源感知、范围细纲修复与首次导入 B1/B4 初始化汇入同一流程 | I139–I140；增强 I149–I151，统一运行 `pnpm run verify:product-flow` |
...
- I1–I150 已交付完整 12 步主流程、来源确认、幕后素材叙事化、C3/C4 安全边界及范围细纲生成修复；混合幕后资料已由 Stage 19 的来源感知流程处理。I151 尚未交付，因此首次导入尚不会自动建立规则/文风初稿。已有正文保真导入已后置为 F2；当前已有正文仍只能拆纲或手工录入，不能保真进入 C5。
