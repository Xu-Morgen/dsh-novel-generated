...
- 权威设计文档：`docs/novel-creation-tool-design.md`（v2.9）
- 权威开发计划：`docs/novel-creation-tool-development-plan.md`（v2.9；I1–I105 已完成，当前执行 I106，最终主流程收口至 I140）
- 需求与验收矩阵：`docs/novel-creation-tool-requirements.md`（v2.9）
...
> **交付状态**：I1–I105 已完成，当前执行 I106。上述 12 步是 v2.9 的最终产品目标；细纲生成范围、统一定稿、全书完成门禁、单文件合稿和主流程界面由 I133–I140 收口，不能把计划中的能力表述为当前已经可用。

### 完整产品级测试流程

| 步骤 | 必须观察到的作者结果 | 主要迭代证据 |
| --- | --- | --- |
| 1–2 导入并确认大纲 | 支持思路/梗概/长稿；候选在确认前零写，非空作品不被静默覆盖 | I37–I38、I50–I53、I119–I120 |
...
| 主流程整体 | 默认进入作者流程，技术工具不干扰主线；固定 fake LLM 端到端覆盖成功、拒绝、陈旧和失败路径 | I139–I140，最终运行 `pnpm run verify:product-flow` |
...
- I1–I105 已交付候选预览/裁决、C5 编辑和细纲绑定，但完整 12 步主流程仍在 Stage 18 实施；当前不能把 README 中的最终流程当作全部已经可用。
- `novel_continue decision=accept` 属兼容快捷路径，不代表 I135–I136 规划的“接受为草稿 → 作者微调 → 最终正文分析 → 一次确认定稿”。需要作者最终文本驱动故事状态时，应等待统一定稿主路径，不要把快捷落盘当作等价验收。
